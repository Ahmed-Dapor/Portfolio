...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('ahmeddapor185@gmail.com');
    setCopied(true);
  }
    ...
    >
      ...
      <div className={`${id === 6 && 'flex justify-center'} h-full'`}>
        ...
        <div className={`absolute right-0 -bottom-5 ${id === 5 && 'w-full opacity-80'}`}>
          {spareImg && (
            <img
              src={spareImg}
              alt={spareImg}
              className={'object-cover , object-center w-full h-f'}
            />
          )}
        ...
        <div className={cn(
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2 mt-16">
              ...
              <div className="flex flex-col gap-3 lg:gap-3">
                <span className="py-6 px-3 rounded-lg text-center bg-[#10132E]"/>
                {['JavaScript' , 'Bootstrab' , 'TailwindCss'].map((item) => (
                  <span key={item} className="py-2 lg:py-3 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                    {item}
                  </span>
                ))}
